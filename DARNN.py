...
class GRU(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers, sequence_length,dropout):
        ...
        # num_trials x samples x features x sequence_length
        self.fc = nn.Linear(hidden_size, output_size)
        self.relu = nn.ReLU()
     
    def forward(self, x, h0, seqMode):
        out, h0 = self.gru(x, h0)
        if seqMode == 1:
            out = self.fc(self.relu(out)) #seq2seq
        else:
            out = self.fc(self.relu(out[:,-1])) # seq2one
        return out, h0
...
class TransformerModel(nn.Module):
    ...
    def forward(self,src,seqMode):
        ...
        if seqMode == 1:
            ...
            output = self.decoder(output[:,-1]) # seq2one
        return output
...
# NOTE:
    # if GRU does not work, switch to nn.GRUCell
...
output_size = 1
num_layers = 2
hidden_size = 512       # [4,8,16,32,64,128,256,512]
sequence_length = 10
step_size = 1
...
nhid = hidden_size # the dimension of the feedforward network model in nn.TransformerEncoder
nlayers = num_layers # the number of nn.TransformerEncoderLayer in nn.TransformerEncoder
nhead = 2 # the number of heads in the multiheadattention models

# DA-RNN Hyperparameters
P = 64              # number of decoder LSTM units
...
# Initialize Model
model = DARNN(input_size, hidden_size, P, sequence_length).to(device)
...
for epoch in range(num_epochs):
    ...
    for (idx, batch) in loop: # in enumerate(train_loader)
        ...
        # Update Progress Bar
        loop.set_description(f"DA-RNN Epoch [{epoch + 1}/{num_epochs}]")
...
model.load_state_dict(torch.load('DARNN_checkpoint.pt'))
...
PATH = os.path.join(cwd,'DA-RNN Results',directory,filename) 
os.makedirs(os.path.join(cwd, 'DA-RNN Results',directory)) 
...
filename = f'DA-RNN {timestr[:-2]}.pt'
PATH = os.path.join(cwd,'DA-RNN Results',directory,filename) 
...
for i in range(num_trials):
    ...
    train_out = model(train_x.squeeze().float()).detach().numpy()
    val_out = model(val_x.squeeze().float()).detach().numpy()
    test_out = model(test_x.squeeze().float()).detach().numpy()
...
# PATH = os.path.join(cwd,'Results',directory,filename)
