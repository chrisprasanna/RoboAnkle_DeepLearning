...
FFN Prosthetic Ankle Torque Predictor 
...
X = np.zeros((num_trials, num_timestepsPerTrial, sequence_length, num_features))
...
# Initialize Data
model = FFN(input_size, hidden_size, output_size, num_layers, dropout).to(device)
...
for epoch in range(num_epochs):
    ...
    for (idx, batch) in loop: # in enumerate(train_loader)
        ...
        # Update Progress Bar
        loop.set_description(f"FFN Epoch [{epoch + 1}/{num_epochs}]")
...
model.load_state_dict(torch.load('FFN_checkpoint.pt'))
...
PATH = os.path.join(cwd,'FFN Results',directory,filename) 
os.makedirs(os.path.join(cwd, 'FFN Results',directory)) 
...
for i in range(num_trials):
    ...
    train_out = model(torch.flatten(train_x,start_dim=2).squeeze().float())
    val_out = model(torch.flatten(val_x,start_dim=2).squeeze().float())
    test_out = model(torch.flatten(test_x,start_dim=2).squeeze().float())
